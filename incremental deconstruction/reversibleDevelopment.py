...
def calcTerminalFitness(tree_path, y0Values, y1Values, ywValues, T, beta, psi_weightingParam, batchSize):
    ...
    for i in pbar(range(stepsize, len(trees) + stepsize, stepsize)):
        if i >= len(trees):
        ...
        key_vallist = {(a, trees[str(a)]) for a in range(i - stepsize, endCondition, 1)}

        identifiers, allTrees = zip(*key_vallist)

        results = pool.map(func_star,
                           zip(allTrees, identifiers, itertools.repeat(y0Values), itertools.repeat(y1Values),
...
def run(pE0, pC0E0, T, argumentR, argumentP, kwUncertainTime, beta, psi_weightingParam, batchSize, dataPath):
    ...
    for t in pbarTimer(tValues):
        ...
        for i in pbarTimeStep(range(stepsize, len(trees) + stepsize, stepsize)):
            if i >= len(trees):
                endCondition = len(trees)
            else:
                endCondition = i
...
if __name__ == '__main__':
    ...
    optimalPolicyFlag = True #would you like to compute the optimal policy
    preparePlottingFlag = True# would you like to prepare the policy for plotting?
    ...
    mainPath = "/home/nicole/Projects/Results - reversible development model/setupRun"
    ...
    cueValidityC0E0Arr = [0.55,0.75,0.95]#['increasing','triangular', 'decreasing']

    # depending on the memory capacity of your system you might consider choosing a smaller batch size
    batchSize = 10000  # how large should parallalizable units be; was 10000
    ...
    dataPath = '/mnt/f28c380a-3bef-40eb-8061-de162309442d/programming/reversible development/'
