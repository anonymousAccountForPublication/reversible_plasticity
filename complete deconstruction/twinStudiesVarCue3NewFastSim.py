...
def plotTriangularPlotsOld(tValues, priorE0Arr, cueValidityArr, maturePhenotypes, T, twinResultsPath, env):
    ...
    for cueVal in cueValidityArr:
        ...
        for pE0 in priorE0Arr:
            ...
            maturePhenotypesCurr = np.transpose(
                [maturePhenotypesCurrAll[:, 0:2].sum(axis=1), maturePhenotypesCurrAll[:, 2:4].sum(axis=1),
            ...
            if isinstance(cueProbabilities, list) or isinstance(cueProbabilities, np.ndarray):
                ...
                area2 = np.array(uniqueFrac) * float(300)


            else:
                area2 = (uniqueCounts / float(numAgents)) * 300

            # this one would be scalling according to area
            tax.scatter(unique[:, permuteIdx], marker="o", s=area2, facecolors='black', colormap=None)
            ...
            if jx == int(len(priorE0Arr) / 2) and ix == int(len(cueValidityArr) / 2):
                fontsize = 20
                tax.right_corner_label("Construction", fontsize=fontsize, offset=-0.15)
                tax.top_corner_label("waiting", fontsize=fontsize)
                tax.left_corner_label("Deconstruction", fontsize=fontsize, offset=-0.15)
                tax._redraw_labels()
